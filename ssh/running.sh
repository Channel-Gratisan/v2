...
echo -e "SCRIPT VPS BY MasWayVPN"
echo "-------------------------------------------------------------------------------"
...
if [ "${status_text}" == "active" ]
then
echo -e " Xray NONE TCP     : Xray NONE TLS Service is "$green"running"$NC""
else
echo -e " Xray NONE TCP     : Xray NONE TLS Service is "$red"not running (Error)"$NC""
fi
status="$(systemctl show xray@vmesstls.service --no-page)"
status_text=$(echo "${status}" | grep 'ActiveState=' | cut -f2 -d=)
if [ "${status_text}" == "active" ]
then
echo -e " Xray TCP          : Xray Vmess TLS Service is "$green"running"$NC""
else
echo -e " Xray TCP          : Xray Vmess TLS Service is "$red"not running (Error)"$NC""
fi
...
echo -e "${green}Report to MasWayVPN @MasWayVPN"
